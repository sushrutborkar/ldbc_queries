...
MATCH (person:Person)-[:KNOWS]-(friend:Person)<-[:HAS_CREATOR]-(comment:Message)-[:REPLY_OF]->(post:Message),
      (post)-[:M_HAS_TAG]->(tag:Tag)-[:HAS_TYPE]->(:TagClass)-[:IS_SUBCLASS_OF{1,5}]->(tagClass:TagClass)
WHERE person.id = 933 AND tagClass.name = 'Person' AND comment.isPost = false AND post.isPost = true
GROUP BY friend
GROUP AS g
SELECT 
    friend.id,
